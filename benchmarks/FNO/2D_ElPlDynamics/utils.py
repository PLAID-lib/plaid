...
class TemporalFractureReader(Dataset):
    def __init__(self,data_path,processes_number,step_definition=1,subset=None):
        ...
        dataset._load_from_dir_(savedir=data_path,verbose=True,processes_number=processes_number,ids=subset)
    ...
    def get_fields(self,index_config,index_timestep):
        ...
        fields = np.stack((sample.get_field(name="U_x",time=index_timestep*self.timedelta),sample.get_field(name="U_y",time=index_timestep*self.timedelta),sample.get_field(name="EROSION_STATUS",time=index_timestep*self.timedelta),sample.get_field(name="Signed_Distance",time=index_timestep*self.timedelta)),axis=1)
        return fields
    ...
    def __getitem__(self,index):
        ...
        if self.subset is not None:
            if index_config not in self.subset:
                raise IndexError("Sample has not been loaded in this dataset, include it in the subset or ")

        input = self.get_fields(index_config,index_timestep)
        output = self.get_fields(index_config,index_timestep+self.step_definition)
        return torch.permute(torch.tensor(input).view(150*2+1,150+1,4),(2,0,1)),torch.permute(torch.tensor(output).view(150*2+1,150+1,4),(2,0,1))
    def __len__(self):
