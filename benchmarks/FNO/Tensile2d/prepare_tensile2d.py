...
size=100
rec_mesh = Tetrahedrization(CreateConstantRectilinearMesh([size*2+1,size*2+1], [-1,-1], [1/size, 1/size]))
...
for sample_index in tqdm(range(nSamples)):
    ...
    for scalar_name in scalar_names:
        old_scalar= sample.get_global( name=scalar_name)
        new_sample.add_global(scalar_name, old_scalar)
