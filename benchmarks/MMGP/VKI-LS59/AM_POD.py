...
class PolynomialManifoldApproximation:
    def __init__(self, polynomial_order, r, q=None, podtype = 'am', reg_ls=1e-3, reg_nt=None, tol_rot=1e-6, tol_nt=1e-8, max_iter_rot=100, max_iter_nt=None, n_jobs=-1, verbose=False):
        """
        ...
        if polynomial_order == 1 and podtype != 'pod':
            if verbose: print(f"WARNING: podtype is set to <{podtype:s}> but polynomial_order is <{polynomial_order:1}>. A podtype <pod> will be used instead")
            podtype = 'pod'
    ...
    def _generate_combinations(self, coefficients):
        ...
        combs = list(combinations_with_replacement(range(coeff_dim), 2))  # Polynomial degree 2
    ...
    def score(self,S_test):
        ...
        return abs(1-np.linalg.norm(self.V @ S_reduced_test + self.Vb @ self.psi @ W_test, ord='fro') / norm_S_test)
    ...
    def fit(self, S, S_test = None):
            ...
            """
            S = S.T
            if S_test is not None: S_test = S_test.T
            assert self.r < S.shape[1]
            if self.q is not None: assert self.q < S.shape[1]
            ...
            if self.verbose: print(f'POD computed cumulated expected variance : {np.cumsum(sig**2)[self.r] / np.sum(sig**2):.4f}')
            if self.verbose: print(40*"=")
            if not self.is_start:
                ...
                self.psi = np.linalg.lstsq(W @ W.T + self.reg_ls * np.eye(W.shape[0]), (self.Vb.T @ S_centered @ W.T).T , rcond=None)[0].T # (q x Ns)
                polynomial_error = 1-np.linalg.norm(self.V @ S_reduce + self.Vb @ self.psi @ W, ord='fro') / norm_S
                SW = np.concatenate([S_reduce.T, (self.psi @ W).T], axis=1)
                old_error = np.copy(polynomial_error)
                if self.verbose: print(f'---> compute polynomial correction {(time.time()-now):.4f} seconds')
                if self.type == 'pod':
                    self.psi *=0
                    self.is_fitted = True
                    return self
                ...
                now = time.time()
                W = self._generate_combinations(S_reduce)
                psi = np.linalg.lstsq(W @ W.T + self.reg_ls * np.eye(W.shape[0]), (Vb.T @ S_centered @ W.T).T , rcond=None)[0].T # (q x Ns)
                polynomial_error = 1-np.linalg.norm(V @ S_reduce + Vb @ psi @ W, ord='fro') / norm_S
                if self.verbose: print(f'---> compute polynomial correction {(time.time()-now):.4f} seconds')
                ...
                W = self._generate_combinations(S_reduce)
                SW = np.concatenate([S_reduce.T, (self.psi @ W).T], axis=1)
                old_error = 1-np.linalg.norm(self.V @ S_reduce + self.Vb @ self.psi @ W, ord='fro') / norm_S
                if self.verbose: print(f'---> restore last AM iteration {(time.time()-now):.4f} seconds')

            if S_test is not None:
                S_reduced_test = self.reduce(self._center_data(S_test))
                W_test = self._generate_combinations(S_reduced_test)
                old_error_test =  abs(1-np.linalg.norm(self.V @ S_reduced_test + self.Vb @ self.psi @ W_test, ord='fro') / norm_S_test)
                if self.verbose: print(f'---> pod {error_pod:.4e} vs polynomial {polynomial_error:.4e} vs AM-polynomial {old_error:.4e} vs test {old_error_test:.4e}')
            else:
                if self.verbose: print(f'---> pod {error_pod:.4e} vs polynomial {polynomial_error:.4e} vs AM-polynomial {old_error:.4e}')

            if self.verbose: print(40*"=")
            if self.verbose: print('initialization completed')
            if self.verbose: print(40*"=")

            for iter_loop in range(self.max_iter_rot):
                if self.verbose: print(f'AM iteration {self.iter_step}')
                ...
                if self.verbose: print(f'---> solve procrustes problem in {(time.time()-now):.4f} seconds')
                ...
                if self.verbose: print(f'---> solve levenberg marquardt problem in {(time.time()-now):.4f} seconds')
                ...
                now = time.time()
                self.psi = np.linalg.lstsq(W @ W.T + self.reg_ls * np.eye(W.shape[0]), (self.Vb.T @ S_centered @ W.T).T , rcond=None)[0].T # (q x Ns)
                if self.verbose: print(f'---> least square problem solved in {(time.time()-now):.4f} seconds')

                # Step 5: Compute SW and approximation error
                SW = np.concatenate([S_reduce.T, (self.psi @ W).T], axis=1) # (Ns x [r+q])
                new_error = 1-np.linalg.norm(self.V @ S_reduce + self.Vb @ self.psi @ W, ord='fro') / norm_S
                if S_test is None:
                    if self.verbose: print(f'---> pod {error_pod:.4e} vs polynomial {polynomial_error:.4e} vs AM-polynomial {new_error:.4e}' )
                else:
                    S_reduced_test = self.reduce(self._center_data(S_test))
                    W_test = self._generate_combinations(S_reduced_test)
                    new_error_test =  abs(1-np.linalg.norm(self.V @ S_reduced_test + self.Vb @ self.psi @ W_test, ord='fro') / norm_S_test)
                    if self.verbose: print(f'---> pod {error_pod:.4e} vs polynomial {polynomial_error:.4e} vs AM-polynomial {new_error:.4e} vs test {new_error_test:.4e}')

                if S_test is not None and (new_error_test > old_error_test):
                    if self.verbose: print('early stop criteria')
                    ...
                    if self.verbose: print('converged criteria')
                    self.is_fitted = True
                    break
    ...
    def _levenberg_marquardt(self, S, Omega):
        ...
        def optimize_column(i):
            ...
            result = least_squares(
                ...
                method='trf',
                max_nfev=self.max_iter_nt*S.shape[0]*self.polynomial_order if self.max_iter_nt is not None else None,
                ftol=self.tol_nt,
                xtol=self.tol_nt,
                gtol=self.tol_nt,
            )
    ...
    def _residu(self, coefficients, S_col, Omega):
        ...
        g = self._generate_combinations(coefficients)  # Generate polynomial combinations
        X = np.concatenate([coefficients, self.psi @ g], axis=0)  # Construct the matrix

        resid = Omega.T @ S_col.reshape(-1, 1) - X  # Compute the residual for this column
    ...
    def _jacobian(self, coefficients, S_col, Omega):
        ...
        J_g = self._compute_combinations_jacobian(coefficients)  # Custom function to compute derivatives
        ...
        if self.reg_nt is None or self.reg_nt == 0.:
            return J
        else:
            JtJ = J.T @ J  # Shape (r, r)
            ...
            regularized_JtJ = JtJ + self.reg_nt * np.diag(diag_JtJ)  # Regularized J.T @ J
    ...
    def _compute_combinations_jacobian(self, coefficients):
        ...
        J = np.zeros((num_combinations, coeff_dim, coefficients.shape[1]))  # Jacobian storage

        # Compute derivatives for each column (snapshot)
        for idx, snapshot in enumerate(coefficients.T):  # Iterate over snapshots
            for row_idx, combo in enumerate(combs):  # Iterate over polynomial combinations
                ...
                for j in range(coeff_dim):  # Iterate over coefficients
                    if j in combo:
                        ...
                        partial_derivative = w_i / snapshot[j] * combo.count(j) if snapshot[j] != 0 else 0
