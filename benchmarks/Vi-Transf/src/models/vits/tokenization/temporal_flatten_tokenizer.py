    ...
    def preprocess(self, dataset: List[List[Data]], seed: int):
        ...
        for n, sample_list in enumerate(dataset):
            for sample in sample_list:
                ...
                sample.padded_community_orders = first_samples_dataset[n].padded_community_orders
                sample.community_reverse_orders = first_samples_dataset[n].community_reverse_orders

                tokens = rearrange(sample.x[sample.padded_community_orders], "t n d -> t (n d)")
                cross_domain_padded_token, attn_mask = pad_subdomains(tokens, n_tokens_per_sim)
                sample.tokens = cross_domain_padded_token.unsqueeze(0)
                sample.attn_mask = attn_mask.unsqueeze(0)

                if hasattr(sample, "output_fields"):
                    expanded_output_fields = rearrange(sample.output_fields[sample.padded_community_orders], "t n d -> t (n d)")
                    cross_domain_padded_expanded_output_fields, _ = pad_subdomains(expanded_output_fields, n_tokens_per_sim)
