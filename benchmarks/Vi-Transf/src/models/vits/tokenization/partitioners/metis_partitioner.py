...
def torch_geometric_to_metis_format(data: Data):
    ...
    for source, target in zip(data.edge_index[0, :].numpy(), data.edge_index[1, :].numpy()):
...
class MetisPartitioner(Partitioner):
    def __init__(self, n_vertices_per_subdomain: int=None, processes_number: int=1, absolute_tol: int=1, relative_tol: float=0.05):
    ...
    def partition(self, dataset: list[Data], seed: Optional[int]=None) -> np.ndarray:
        ...
        if seed is not None:
            ...
            seed_vector = [rng_generator.randint(0, 2**30 - 1) for _ in range(len(dataset))] # metis doesn't accept seeds that are too large
        ...
        if self.processes_number > 1:
            ...
            for i, data in enumerate(tqdm(dataset, desc="Partitioning dataset with METIS", total=len(dataset))):
                partitioned_data = _partition_single(data, self.n_vertices_per_subdomain, seed_vector[i], self.absolute_tol, self.relative_tol)
                partitioned_dataset.append(partitioned_data)

        return partitioned_dataset


def _partition_single(data: Data, npoints, seed=None, absolute_tol: int=1, relative_tol: float=0.05) -> torch.Tensor:
    if seed is None: seed=random.randint(0, 2**30 - 1)
    options = Options(seed=seed)

    n_sim_points = data.x.shape[0]
    n_subdomains = npoints_to_nparts(npoints=npoints, n_sim_points=n_sim_points, absolute_tol=absolute_tol, relative_tol=relative_tol)
