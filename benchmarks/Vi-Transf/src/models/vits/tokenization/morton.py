...
def compute_3dmorton_order(point_cloud):
    ...
    morton_code = ijk_split_by_3[:, 0] | ijk_split_by_3[:, 1] << 1 | ijk_split_by_3[:, 2] << 2
...
def compute_morton_order(point_cloud):
    if point_cloud.shape[1]==2:
        ...
        raise ValueError(f"Only works for 2d or 3d pointclouds, this won't work in dimension {point_cloud.shape[1]}")
