...
def elasto_plasto_dynamics_sample_to_geometric(sample: Sample, sample_id: int, problem_definition: ProblemDefinition) -> Data:
    ...
    output_fields = np.vstack((sample.get_field("U_x", time=timestep_list[0]), sample.get_field("U_y", time=timestep_list[0]))).T
    data_list = []

    vertices_torch  = torch.from_numpy(vertices).to(torch.float32) #torch.tensor(vertices, dtype=torch.float32)
    edge_index      = torch.from_numpy(edge_index).to(torch.long) #torch.tensor(edge_index, dtype=torch.long)

    for t0, t1 in zip(timestep_list[:-1], timestep_list[1:]):
        output_fields_t1 = output_fields
        if output_fields_t1[0, 0] is not None:
            output_fields = np.vstack((sample.get_field("U_x", time=t1), sample.get_field("U_y", time=t1))).T
                
            input_scalars = np.array([t0])
            input_fields = np.column_stack((vertices, sdf, projection_vectors, output_fields_t1))

            # torch tensor conversion
            input_scalars   = torch.from_numpy(input_scalars).to(torch.float32).reshape(1, -1) #torch.tensor(input_scalars, dtype=torch.float32).reshape(1, -1)
            input_fields    = torch.from_numpy(input_fields).to(torch.float32) #torch.tensor(input_fields, dtype=torch.float32)
            ...
            if None not in output_fields:
                output_fields   = torch.from_numpy(output_fields).to(torch.float32) # torch.tensor(output_fields, dtype=torch.float32)
