...
class StandardScaler(Scaler):
    ...
    def transform(self, dataset: List[Data]):
        ...
        for data in dataset:
            ...
            if isinstance(data.edge_attr, torch.Tensor) and len(data.edge_attr.ravel()) > 0:
            ...
            if isinstance(data.edge_weight, torch.Tensor) and len(data.edge_weight.ravel()) > 0:
    ...
    def inverse_transform(self, dataset: List[Data] | Data):
        ...
        for data in dataset:
            ...
            if isinstance(data.edge_weight, torch.Tensor) and len(data.edge_weight.ravel()) > 0:
                edge_weight_dtype = data.edge_weight.dtype
                data.edge_weight = torch.as_tensor(
                    self.edge_weight_scaler.inverse_transform(data.edge_weight.reshape(-1, 1)),
                    dtype=edge_weight_dtype,
                ).reshape(-1)
            ...
            if hasattr(data, 'scalars_prediction') and isinstance(data.scalars_prediction, torch.Tensor) and len(data.scalars_prediction.ravel()) > 0:
    ...
    def inverse_transform_prediction(self, dataset: List[Data]):
        ...
        for data in dataset:
            if hasattr(data, 'fields_prediction') and isinstance(data.fields_prediction, torch.Tensor) and len(data.fields_prediction.ravel()) > 0:
                data.fields_prediction = torch.tensor(
                    self.yf_scaler.inverse_transform(data.fields_prediction.detach().cpu().numpy()),
                    dtype=data.fields_prediction.dtype
                )

            if hasattr(data, 'scalars_prediction') and isinstance(data.scalars_prediction, torch.Tensor) and len(data.scalars_prediction.ravel()) > 0:
                data.scalars_prediction = torch.tensor(
                    self.ys_scaler.inverse_transform(data.scalars_prediction.detach().cpu().numpy().reshape(1, -1)),
                    dtype=data.scalars_prediction.dtype
                )
