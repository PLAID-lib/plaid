...
def create_submission(
    prediction_dataset: list[Data], plaid_dataset: PlaidDataset, save_dir: str = "."
):
    pyg_id_dico = {}
    ...
    output_scalars_names = (
        prediction_dataset[0].output_scalars_names
        if hasattr(prediction_dataset[0], "output_scalars_names")
        else []
    )
    ...
    for i, id in enumerate(ids_test):
        ...
        output_fields_prediction = (
            sample.fields_prediction.detach().cpu().numpy()
            if hasattr(sample, "fields_prediction")
            and sample.fields_prediction is not None
            else None
        )
        output_scalars_predictions = (
            sample.scalars_prediction.reshape(-1).detach().cpu().numpy()
            if hasattr(sample, "scalars_prediction")
            and sample.scalars_prediction is not None
            else None
        )
