...
def quad_cells_to_edges(cells):
    edges = torch.cat([cells[:, 0:2], cells[:, 1:3], cells[:, 2:4], torch.stack((cells[:, 3], cells[:, 0]), dim=-1)], dim=0)
    receivers, _ = torch.min(edges, dim=-1)
    senders, _ = torch.max(edges, dim=-1)
...
def distance_field(mesh, nTag=None):
    ...
    opSkin, _, _  =  GetFieldTransferOp(inputField= field_mesh, targetPoints= mesh.nodes, method="Interp/Clamp" , elementFilter= FO.ElementFilter(dimensionality=dim-1, nTag=nTag) , verbose=False)
...
def get_data(mesh, dataset_name=None, load_fields=True):
    ...
    tag_label_map = {
        "Airfoil": 2, "Holes": 2, "Top": 2,
        "Inlet": 4, "Bottom": 4, "Inflow": 4,
        "Ext_bound": 6, "Outflow": 6,
        "Intrado": 1,
        "Extrado": 3,
        "Periodic_1": 5,
        "Periodic_2": 7
    ...
    edge_function = quad_cells_to_edges if dataset_name == "VKI_LS59" else tri_cells_to_edges
    ...
    node_features_dict = {'distance': distance, 'node_type': node_type, 'mesh_pos': mesh_pos}
...
def load_datasets(dataset_name, dataset_path, split_train_name=None, split_test_name=None, target_field="all_fields"):
    ...
    train_indices, test_indices = read_indices_from_csv(dataset_path, split_train_name, split_test_name)
    ...
    def process_samples(dataset_name, dataset_path, indices, field_names, process_type):
        ...
        for i in track(range(len(indices)), total=len(indices), description=description):
            id_sample = indices[i]
            ...
            X_scalars.append([mesh_data.get_scalar(fn) for fn in in_scalars_names] if in_scalars_names else [])

            # Retrieve output scalars
            out_scalars_names = scalar_output_dict.get(dataset_name, [])
            Y_scalars.append([mesh_data.get_scalar(fn) for fn in out_scalars_names] if out_scalars_names else [])
    ...
    train_data = process_samples(dataset_name, dataset_path, train_indices, field_names, "train")
    test_data = process_samples(dataset_name, dataset_path, test_indices, field_names, "test")

    return train_data, test_data, train_indices, test_indices


class GraphDataset(DGLDataset):
    def __init__(self, args, data, data_type, in_scaler=None, out_scaler=None, fields_min=None, fields_max=None):
        ...
        if data_type == "train":
            ...
            self.input_globals = torch.tensor(self.in_scaler.fit_transform(data["X_scalars"]), dtype=torch.float32) if data["X_scalars"].size > 0 else torch.tensor([])
            self.output_globals = torch.tensor(self.out_scaler.fit_transform(data["Y_scalars"]), dtype=torch.float32) if data["Y_scalars"].size > 0 else torch.tensor([])

            self.fields_min = torch.min(torch.cat(data["Y_fields"], dim=0), dim=0).values
            self.fields_max = torch.max(torch.cat(data["Y_fields"], dim=0), dim=0).values

        elif data_type == "test":
            assert all(v is not None for v in [in_scaler, out_scaler, fields_min, fields_max])

            self.input_globals = torch.tensor(self.in_scaler.transform(data["X_scalars"]), dtype=torch.float32) if data["X_scalars"].size > 0 else torch.tensor([])
            self.output_globals = torch.tensor(self.out_scaler.transform(data["Y_scalars"]), dtype=torch.float32) if data["Y_scalars"].size > 0 else torch.tensor([])

        description = f"🚀 Processing {data_type} graphs"
        for i in track(range(self.num_samples), total=self.num_samples, description=description):
            ...
            differences = (data["X_nodes"][i][src] - data["X_nodes"][i][dst]).clone().detach()
