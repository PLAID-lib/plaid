...
class PlaidTransientDataset(Dataset):
    def __init__(self, plaid_dataset, plaid_problem, split="train", fields=("U_x","U_y")):
        ...
        for sid in self.ids:
            ...
            edge_index = tri_cells_to_edges(torch.tensor(cells, dtype=torch.long)).t().contiguous()

            _, sdf = get_distances_to_borders(sample.get_nodes(), cells)
            sdf       = torch.tensor(sdf, dtype=torch.float32)
            sdf_sine  = sinusoidal_embedding(sdf, num_basis=4, max_coord=4, spacing=0.001)
            angles    = angles_to_planes(pos)
            sph       = torch.cat([
                spherical_harmonics(angles[:,i], l_max=4)[:,1:]
                for i in range(4)
            ], dim=1)

            src, dst  = edge_index
            disp      = pos[src] - pos[dst]
            sqd       = torch.exp(-0.5 * (disp**2).sum(1,keepdim=True)/(5.757861066563731*10))
    ...
    def __getitem__(self, idx):
        ...
        ux_t  = torch.tensor(sample.get_field(self.fields[0], time=self.time_steps[t]),   dtype=torch.float32)
        uy_t  = torch.tensor(sample.get_field(self.fields[1], time=self.time_steps[t]),   dtype=torch.float32)
        ux_tp = torch.tensor(sample.get_field(self.fields[0], time=self.time_steps[t+1]), dtype=torch.float32)
        uy_tp = torch.tensor(sample.get_field(self.fields[1], time=self.time_steps[t+1]), dtype=torch.float32)
...
def compute_minmax_scaler(train_ds, args, device):
    loader = GraphDataLoader(train_ds, batch_size=args.batch_size,
...
def save_checkpoint(path, model, optimizer, epoch, rank):
    if rank == 0:
        state = {
            'epoch': epoch,
            'model_state': model.module.state_dict() if isinstance(model, torch.nn.parallel.DistributedDataParallel)
...
def evaluate(model, device, test_ds, scaler, args, rank):
    ...
    with torch.no_grad():
        for sim_idx, sid in enumerate(test_ds.ids):
            ...
            # Init u_pred at t0
            ux0 = torch.tensor(sample.get_field("U_x", time=test_ds.time_steps[0])).to(device)
            uy0 = torch.tensor(sample.get_field("U_y", time=test_ds.time_steps[0])).to(device)
            ...
            for t in range(test_ds.n_steps):
                ...
                if scaler['type'] == 'minmax':
                    x0 = (u_pred - scaler['min_x']) / (scaler['max_x'] - scaler['min_x'])
                ...
                # Ground truth at t+1
                ux_tp = torch.tensor(sample.get_field("U_x", time=test_ds.time_steps[t+1])).to(device)
                uy_tp = torch.tensor(sample.get_field("U_y", time=test_ds.time_steps[t+1])).to(device)
                u_tp  = torch.stack([ux_tp, uy_tp], dim=-1)
...
def main_worker(args):
    ...
    # Load data & problem
    if rank==0: print("📂 Loading Plaid data")
    t0 = time.perf_counter()
    plaid_ds = PlaidDataset(); plaid_ds._load_from_dir_(args.data_dir, verbose=(rank==0))
    problem  = ProblemDefinition(); problem._load_from_dir_(args.problem_dir)
    t1 = time.perf_counter()
    if rank==0:
        print(f"✔️ [Data load] {t1-t0:.2f}s — train sims={len(problem.get_split('train'))}, test sims={len(problem.get_split('test'))}")
    ...
    if args.scaler == 'minmax':
        if rank==0: print("📏 Computing Min-Max scaler")
        t0 = time.perf_counter()
        tmp = compute_minmax_scaler(train_ds, args, device)
        dist.broadcast_object_list([tmp], src=0)
        scaler = tmp
        t1 = time.perf_counter()
        if rank==0: print(f"[Scaler minmax] {t1-t0:.2f}s → {scaler}")
    elif args.scaler == 'standard':
        if rank==0: print("📏 Computing Standard scaler")
        t0 = time.perf_counter()
        tmp = compute_standard_scaler(train_ds, args, device)
        dist.broadcast_object_list([tmp], src=0)
        scaler = tmp
        t1 = time.perf_counter()
        if rank==0: print(f"✔️ [Scaler std] {t1-t0:.2f}s → {scaler}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            if scaler['type']=='minmax':
                g.ndata['x'][:,:2] = (g.ndata['x'][:,:2] - scaler['min_x']) / (scaler['max_x'] - scaler['min_x'])
                g.ndata['y']       = (g.ndata['y']       - scaler['min_y']) / (scaler['max_y'] - scaler['min_y'])
            elif scaler['type']=='standard':
                g.ndata['x'][:,:2] = (g.ndata['x'][:,:2] - scaler['mean_x']) / scaler['std_x']
        ...
        if rank==0:
            ...
            print(f"✅ [Epoch {epoch+1}/{args.epochs}] loss={avg_loss:.9f} — time={ep1-ep0:.2f}s")

            os.makedirs(args.ckpt_path, exist_ok=True)

            if (epoch+1) % args.save_interval == 0:
                save_checkpoint(f"{args.ckpt_path}/epoch_{epoch+1}.pth", model, optimizer, epoch, rank)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--aggregation",type=str, choices=["sum","mean","max"], default="sum")
