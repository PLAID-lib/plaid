...
from typing import Iterator, Literal, Optional, Union, Sequence
...
class Dataset:
    ...
    def load(self, path: Union[str, Path]):
        ...
        if path.is_file():
            ...
            try:
                ...
                self._ds = datasetdict[self._split]
                self._conv = converterdict[self._split]
            ...
            raise FileNotFoundError(f"Error! path '{self.path}' does not exist")
    ...
    def from_training_split(
        ...
        split, indices = problem_definition.get_training_split()
    ...
    @classmethod
    def from_evaluating_split(
        cls,
        path: Path,
        pb_def_name: str = "PLAID_benchmark",
    ) -> "Dataset":
        """Create a Dataset from the evaluating split defined in the problem definition.
        ...
        Returns:
            Dataset instance loaded from the evaluating split.
        """
        problem_definition = ProblemDefinition(path=path, name=pb_def_name)
        split, indices = problem_definition.get_evaluating_split()
    ...
    def set_transform_stage(self, transform_stage: str):
        """Set the transformation stage and initialize features accordingly.

        Args:
            transform_stage: The transformation stage to set. For training stage,
                must be "transform" or "fit_transform". For evaluating stage,
                must be "transform" or "inverse_transform".
        """
        if self.stage == "training":
            assert transform_stage in ["transform", "fit_transform"], (
                f"transform_stage must be transform or fit_transform, got {transform_stage}"
            )
            self.init_feats = self.problem_definition.get_all_features()
        elif self.stage == "evaluating":
            if transform_stage == "transform":
                self.init_feats = self.problem_definition.get_input_features()
            elif transform_stage == "inverse_transform":
                self.init_feats = self.problem_definition.get_all_features()
            else:
                raise ValueError(
                    f"transform_stage must be transform or inverse_transform, got {transform_stage}"
                )
        else:
            raise ValueError(f"{self.stage} must be training or evaluating")

    def set_ids(self, ids: np.ndarray):
        """Restrict the dataset view to a specific set of sample indices.

        Args:
            ids: Array of integer indices referencing samples in the underlying split.
        """
        self._ids = ids
    ...
    def add_sample(self, sample: Union[Sample,Sequence[Sample]], id: Optional[Union[int,Sequence[int]]] = None) -> Union[int,list]:
        """Add a new :class:`Sample <plaid.containers.sample.Sample>` to the :class:`Dataset <plaid.containers.dataset.Dataset>.`.

        Args:
            sample (Sample) or Sequence[Sample]: The sample or sequence of samples to add.
            id (int or Sequence[int], optional): An optional ID or sequence of IDs for the new samples. If not provided, the IDs will be automatically generated based on the current number of samples in the dataset.

        Raises:
            TypeError: If ``sample`` is not a :class:`Sample <plaid.containers.sample.Sample>`.

        Returns:
            int or list: Id or list of IDs of the new added :class:`Sample <plaid.containers.sample.Sample>`.
        ...
        if isinstance(sample, Sample):
            ...
            if len(sample) != len(id):
                raise ValueError("The length of the list of samples to add and the list of IDs are different")
            if id is None:
                id = list(range(len(self), len(self) + len(sample)))
            return [self.add_sample(id=i, sample=s) for i,s in zip(id,sample)  ]

    def set_sample(self, sample: dict[int, Sample]) -> None:
        """Set the samples of the data set, overwriting the existing ones.
