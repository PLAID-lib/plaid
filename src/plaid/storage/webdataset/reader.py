...
def load_dataset_from_disk(
    ...
    print(">>", [str(local_path / sn)+"/sample_*/*.tar" for sn in split_names])
    tar_files = {sn: sorted(glob.glob(str(local_path / sn)+"/sample_*/*.tar")) for sn in split_names}
    return {sn: wds.WebDataset(tar_files[sn], shardshuffle = False).decode(npy_decoder) for sn in split_names}
