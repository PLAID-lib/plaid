...
from plaid.storage.common.writer import _check_folder, save_infos_to_disk, save_problem_definitions_to_disk, save_metadata_to_disk, push_infos_to_hub, push_problem_definitions_to_hub, push_metadata_to_hub
from plaid.storage.common.reader import load_infos_from_disk, load_problem_definitions_from_disk, load_metadata_from_disk

# HF_DATASETS
from plaid.storage.hf_datasets.bridge import generator_to_datasetdict as generator_to_hf_datasetdict
from plaid.storage.hf_datasets.reader import init_datasetdict_from_disk as init_hf_datasetdict_from_disk
...
def save_to_disk(
    ...
    assert backend in AVAILABLE_BACKENDS, f"backend {backend} not among available ones: {AVAILABLE_BACKENDS}"

    output_folder = Path(output_folder)

    _check_folder(output_folder, overwrite)

    flat_cst, variable_schema, constant_schema, split_n_samples = preprocess(generators, gen_kwargs=gen_kwargs, num_proc=num_proc, verbose=verbose)

    if backend == "hf_datasets":
        hf_datasetdict = generator_to_hf_datasetdict(generators, variable_schema, gen_kwargs=gen_kwargs, processes_number=num_proc)
        save_hf_datasetdict_to_disk(output_folder, hf_datasetdict, num_proc=num_proc)

    elif backend == "zarr":
        save_zarr_datasetdict_to_disk(output_folder, generators, variable_schema, gen_kwargs=gen_kwargs, num_proc=num_proc, verbose=verbose)

    elif backend == "cgns":
        save_cgns_datasetdict_to_disk(output_folder, generators, gen_kwargs=gen_kwargs, num_proc=num_proc, verbose=verbose)

    else:
        raise ValueError(f"backend {backend} not implemented")

    save_metadata_to_disk(output_folder, flat_cst, variable_schema, constant_schema)
...
def push_to_hub(
    ...
    push_metadata_to_hub(repo_id, flat_cst, variable_schema, constant_schema)
