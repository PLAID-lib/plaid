...
# TODO:
# - LazyGPR (ou LazyWrappedPlaidSklearnRegressor)
...
class WrappedPlaidSklearnTransformer(PlaidSklearnBlockWrapper, TransformerMixin):
    ...
    def transform(self, dataset: Dataset):
        ...
        X_transformed = X_transformed.reshape((len(dataset), len(self.params['out_features_identifiers']), -1))

        dataset_transformed = dataset.from_tabular(X_transformed, self.params['out_features_identifiers'], restrict_to_features = False)
    ...
    def inverse_transform(self, dataset: Dataset):
        ...
        X = dataset.get_tabular_from_homogeneous_identifiers(self.params['out_features_identifiers'])
        assert X.shape[1] == len(self.params['out_features_identifiers']), "number of features not consistent between generated tabular and out_features_identifiers"
        X = np.squeeze(X)

        X_inv_transformed = self.sklearn_block_.inverse_transform(X)
        X_inv_transformed = X_inv_transformed.reshape((len(dataset), len(self.params['in_features_identifiers']), -1))

        dataset_inv_transformed = dataset.from_tabular(X_inv_transformed, self.params['in_features_identifiers'], restrict_to_features = False)
...
class PlaidTransformedTargetRegressor(BaseEstimator, RegressorMixin):
    ...
    def score(self, dataset_X, dataset_y = None):
        ...
        pred_features_identifiers = self.transformer[0].params['in_features_identifiers']

        scores = []
        for feat_id in pred_features_identifiers:
