...
new_sample_mult_mesh._meshes.set_meshes(meshes_dict)

print(f"{new_sample_mult_mesh._meshes.get_all_mesh_times() = }")
...
print(f"{sample._meshes.get_points() = }")  # same as get_nodes
print(f"{sample._meshes.get_vertices() = }")  # same as get_nodes
...
tmp_sample._meshes.add_tree(tree)

print("element connectivity = \n", f"{tmp_sample._meshes.get_elements()}")
...
# %%
# Get base names
bases_names = sample._meshes.get_base_names()
# Get full base path
full_bases_names = sample._meshes.get_base_names(full_path=True)
...
base_name = sample._meshes.get_base_names()[0]
# Get base node
base_node_content = sample._meshes.get_base(base_name)
...
print(f"{sample._meshes.has_base(base_name) = }")
print(f"{sample._meshes.has_base('unknown_base_name') = }")
...
zones_names = sample._meshes.get_zone_names(base_name)
# Get full path of zones associated with the first base
full_zones_names = sample._meshes.get_zone_names(base_name, full_path=True)
...
# Get base node
zone_node_content = sample._meshes.get_zone(zone_name, base_name)
...
# Get the first zone name from a base name
zone_name = zones_names[0]
z_type = sample._meshes.get_zone_type(zone_name, base_name)
...
print(f"{sample._meshes.has_zone(zone_name, base_name) = }")
print(f"{sample._meshes.has_zone('unknown_zone_name', base_name) = }")
...
print(f"{sample._meshes.get_all_mesh_times() = }")

# Add one CGNS tree at time 1.
sample._meshes.add_tree(tree, 1.0)
...
for b_name in bases_names:
    zones_names = sample._meshes.get_zone_names(b_name)
    full_zones_names = sample._meshes.get_zone_names(b_name, full_path=True)
    print(f" - Base : {b_name}")
    for z_name, f_z_name in zip(zones_names, full_zones_names):
        print(
            f"    - {z_name} -> type: {sample._meshes.get_zone_type(z_name, b_name)} | full: {f_z_name}"
        )
...
print(f"{sample._meshes.get_all_mesh_times() = }")
print(f"{sample._meshes.get_time_assignment() = }", end="\n\n")
...
# %%
# Reset default time
sample._meshes._default_active_time = None
...
print(f"{sample._meshes.get_topological_dim('new_base', 0.0) = }")
print(f"{sample._meshes.get_physical_dim('new_base', 0.0) = }")

# %%
# Attempting to get a base when the default base is not set, and there are multiple bases available.
print(f"{sample._meshes.get_base_names() = }", end="\n\n")
try:
...
print(f"{sample._meshes.get_time_assignment() = }")
print(f"{sample._meshes.get_base_assignment() = }", end="\n\n")

# Print the topological and physical dim for the default base == 'SurfaceMesh'
print(f"{sample._meshes.get_topological_dim() = }")
print(f"{sample._meshes.get_physical_dim() = }")

# %%
# If base is specified as an argument in a function, it takes precedence over the default base.
print(
    f"{sample._meshes.get_physical_dim('new_base') = }"
...
sample._meshes._default_active_time = None
sample._meshes._default_active_base = None
...
print(f"{sample._meshes.get_zone_type('TestZoneName', 'SurfaceMesh') = }")
print(f"{sample._meshes.get_zone_type('new_zone', 'SurfaceMesh') = }")
...
print(f"{sample._meshes.get_zone_names() = }", end="\n\n")
try:
    sample._meshes.get_zone_assignment()
except KeyError as e:
    print(str(e))
