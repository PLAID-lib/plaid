...
class Test_Dataset:
    ...
    def test_get_scalars_to_tabular_multidimensional_scalars(self, dataset, tabular, scalar_names):
        dataset.add_tabular_scalars(tabular, scalar_names)
        ...
        tab = dataset.get_scalars_to_tabular(scalar_names=["test_scalar_1D"], as_nparray=True)
        assert tab.shape == (len(tabular), 4)
        assert np.all(tab == test_scalar_1D)
        tab = dataset.get_scalars_to_tabular(scalar_names=["test_scalar_2D_row"], as_nparray=True)
        assert tab.shape == (len(tabular), 3)
        assert np.all(tab == test_scalar_2D_row.squeeze(1))
        tab = dataset.get_scalars_to_tabular(scalar_names=["test_scalar_2D_col"], as_nparray=True)
        assert tab.shape == (len(tabular), 5)
        assert np.all(tab == test_scalar_2D_col.squeeze(2))

        # assert values
        dict_tabular = dataset.get_scalars_to_tabular(scalar_names=["test_scalar_1D", "test_scalar_2D_row", "test_scalar_2D_col"])
        assert np.all(dict_tabular["test_scalar_1D"] == test_scalar_1D)
        assert np.all(dict_tabular["test_scalar_2D_row"] == test_scalar_2D_row.squeeze(1))
        assert np.all(dict_tabular["test_scalar_2D_col"] == test_scalar_2D_col.squeeze(2))
