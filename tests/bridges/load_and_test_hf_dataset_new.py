...
hf_dataset_old = datasets.load_dataset("PLAID-datasets/Tensile2d", split="all_samples")
...
start = time()
hf_dataset_new = datasets.load_dataset(repo_id, split="train_500")
end = time()
...
plaid_dataset = huggingface_bridge.huggingface_dataset_to_plaid(
    hf_dataset_old, ids=train_id, processes_number=24, verbose=True
)
end = time()
...
plaid_dataset_new = huggingface_bridge.huggingface_dataset_to_plaid_new(
    hf_dataset_new, dtypes, cgns_types, processes_number=24
)
end = time()
...
start = time()
hf_dataset_test = datasets.load_dataset(repo_id, split="train_500", streaming=True)
